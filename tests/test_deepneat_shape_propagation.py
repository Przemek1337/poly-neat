...
def test_conv_after_dense_is_rejected_instead_of_changing_the_layer_type() -> None:
    genome = DeepNEATGenome(
        node_genes=(
            LayerNodeGene(node_id=0, layer_type="input"),
            LayerNodeGene(node_id=1, layer_type="output"),
    ...
    with pytest.raises(IncompatibleTensorShapesError):
        propagate_tensor_shapes(
            genome,
            prune_to_nodes_on_an_input_output_path(genome),
            _INPUT_SHAPE,
...
def test_spatial_and_flat_parents_are_rejected_without_an_encoded_adapter() -> None:
    with pytest.raises(IncompatibleTensorShapesError):
        compute_merge_strategy(
            incoming_shapes=[
...
def test_output_merge_downsamples_spatial_parents_before_flattening() -> None:
    strategy = compute_merge_strategy(
        incoming_shapes=[
            TensorShape.spatial(channels=1, height=28, width=28),
            TensorShape.spatial(channels=4, height=14, width=14),
        ],
        target_layer_type="output",
    )
    assert strategy.flatten_inputs
    assert (strategy.pooled_height, strategy.pooled_width) == (14, 14)
    assert strategy.merged_features == 5 * 14 * 14
...
def test_skip_connection_merges_at_the_output() -> None:
    ...
    # The raw 28x28 shortcut is downsampled to the conv parent's 14x14 before
    # both spatial tensors are concatenated and flattened for classification.
