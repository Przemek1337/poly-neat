"""Shared wiring behind the CIFAR-10 benchmark profiles, smoke, pilot and full.
...
and a full run is bound to a frozen v2 lock. CIFAR-10 downloads itself, so unlike
...
from examples.cifar10._protocol import (
    CHANNELS,
    IMAGE_SIDE,
    NUMBER_OF_CLASSES,
...
_PROFILE_DESCRIPTION = "PolyNEAT CIFAR-10 benchmark profile"
...
def run_profile_experiment(
    *,
    ...
    """Run one CIFAR-10 profile end to end through the shared protocol.

    Raises:
        ExecutionLockError: If a full run's configuration, seed, environment or
            dataset identity does not match its frozen lock.
    """
    del data_directory  # CIFAR-10 downloads itself; there is no archive to point at.
...
def run_profile_main(
    ...
    *,
    ...
    """Run one CIFAR-10 profile from the command line via the shared spine."""
...
def parse_profile_cli(argument_list: list[str] | None = None, *, default_config_file_path: Path):
    """Parse the CIFAR-10 profile command line via the shared spine."""
