"""MNIST digit classification with DeepNEAT - PolyNEAT end-to-end demo.

Evolves layer-level CNN topology over full-resolution 28x28 MNIST images
(Miikkulainen et al. 2017's central idea: each genome node is a whole layer
with its own hyperparameters, edges carry tensors, and every genome is
...
inheritance). Fitness is measured on a validation slice carved out of the
training rows; the test split stays untouched by evolution and is used only
for the final report. Population and epoch budgets are far below a
production-scale search so the example finishes on one machine; the best
genome is finally retrained from scratch on train+validation and reported as
classification accuracy on the untouched test split.

The dataset is downloaded once (as the standard Keras ``mnist.npz``) and
cached under ``examples/mnist/data/``.

Run from the repository root:
    uv run python -m examples.mnist.deepneat [--cpu | --gpu]

Artifacts are written to examples/mnist/artifacts/deepneat/:
...
def run_experiment(
    ...
    """Run the full MNIST DeepNEAT experiment once.
...
def main() -> None:
    """Evolve an MNIST DeepNEAT classifier and print all four metrics."""
